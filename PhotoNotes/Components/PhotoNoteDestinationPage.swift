        ...
        NavigationStack {
            List {
                ...
                Section(header: Text("location")) {
                    MapView(coordinates: photoNoteModel.coordinates, title: photoNoteModel.title)
                        .scaledToFit()
                }
            }
...
#Preview {
    struct Preview: View {
        ...
        var photoNoteViewModel: HomeModel
        
        init() {
            self.coordinates = CLLocationCoordinate2D(latitude: 37.7749, longitude: -122.4194)
            self.photoNoteViewModel = HomeModel(title: "hi", photo: Image(systemName: "plus"), description: "hi", coordinates: coordinates)
        }
